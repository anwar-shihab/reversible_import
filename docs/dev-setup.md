...
sudo apt update
sudo apt install python3.11-dev python3.11-venv build-essential mariadb-client libmariadb-dev redis-server nodejs yarn wkhtmltopdf

# Create bench
bench init --frappe-branch version-16 --python python3.11 frappe-bench
# or for v15
...
```bash
bench --site test_site python -m pytest reversible_import/tests
```

## CI

CI uses the same `bench init` workflow. See `.github/workflows/ci.yml`.
